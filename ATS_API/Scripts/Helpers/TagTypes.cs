...
{
    public static ModelTag GetOrCreate(string name)
    {
        if (!_customTagTypes.Contains(name))
        {
            var newTag = ScriptableObject.CreateInstance <ModelTag>();
            newTag.name = name;
            
            _customTagTypes.Add(name);
            SO.Settings.tags = SO.Settings.tags.Append(newTag).ToArray();
            //todo check if this needs syncing
        }

        return SO.Settings.tags.FirstOrDefault(tag => tag.Name == name);
    }
    ...
    internal static IReadOnlyCollection <string> CustomTagTypes => _customTagTypes;
    private static List <string> _customTagTypes = new List <string>();
